...
pub fn main_2_1() {
	...
	let (our_shader, lamp_shader, vbo, cube_vao, cube_positions, light_vao) = unsafe {

		gl::Enable(gl::DEPTH_TEST);

		let our_shader = Shader::new(
			"src/_2_lighting/shaders/1.color.vs", 
			"src/_2_lighting/shaders/1.color.fs"
		);
		...
		(our_shader, lamp_shader, vbo, cube_vao, cube_positions, light_vao)
	};
    ...
    while !window.should_close() {
		...
		unsafe {
			...
			our_shader.use_program();
			our_shader.set_vec3(CStr::from_bytes_with_nul(b"objectColor\0").unwrap(), 1.0, 0.5, 0.31);
			our_shader.set_vec3(CStr::from_bytes_with_nul(b"lightColor\0").unwrap(), 1.0, 1.0, 1.0);
