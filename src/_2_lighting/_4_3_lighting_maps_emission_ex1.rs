...
pub fn main_2_4_3() {
	...
	let (lighting_shader, lamp_shader, vbo, cube_vao, cube_positions, light_vao, diffuse_map, specular_map, emission_map) = unsafe {

		gl::Enable(gl::DEPTH_TEST);

		let lighting_shader = Shader::new(
			"src/_2_lighting/shaders/4.2.lighting_maps_specular.vs", 
			"src/_2_lighting/shaders/4.3.lighting_maps_emission_ex1.fs"
		);
        ...
        lighting_shader.set_int(CStr::from_bytes_with_nul(b"emission\0").unwrap(), 2);


		// uncomment this call to draw in wireframe polygons.
        // gl::PolygonMode(gl::FRONT_AND_BACK, gl::LINE);

		(lighting_shader, lamp_shader, vbo, cube_vao, cube_positions, light_vao, diffuse_map, specular_map, emission_map)
	};
    ...
    while !window.should_close() {
		...
		unsafe {
			...
			lighting_shader.set_float(CStr::from_bytes_with_nul(b"time\0").unwrap(), glfw.get_time() as f32 / 2.0);
