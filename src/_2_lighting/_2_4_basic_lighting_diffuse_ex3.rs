...
pub fn main_2_2_4() {
	...
	let (lighting_shader, lamp_shader, vbo, cube_vao, cube_positions, light_vao) = unsafe {
		...
		let lighting_shader = Shader::new(
			"src/_2_lighting/shaders/2.4.basic_lighting_diffuse_ex3.vs", 
			"src/_2_lighting/shaders/2.4.basic_lighting_diffuse_ex3.fs"
		);
    ...
    while !window.should_close() {
		...
		unsafe {
			...
			lighting_shader.use_program();
			lighting_shader.set_vec3(CStr::from_bytes_with_nul(b"objectColor\0").unwrap(), 1.0, 0.5, 0.31);
			lighting_shader.set_vec3(CStr::from_bytes_with_nul(b"lightColor\0").unwrap(), 1.0, 1.0, 1.0);
			lighting_shader.set_vector3(CStr::from_bytes_with_nul(b"lightPos\0").unwrap(), &light_pos);
