...
pub fn main_2_5_4() {
	...
	let (lighting_shader, lamp_shader, vbo, cube_vao, cube_positions, light_vao, diffuse_map, specular_map) = unsafe {
		...
		(lighting_shader, lamp_shader, vbo, cube_vao, cube_positions, light_vao, diffuse_map, specular_map)
	};
    ...
    while !window.should_close() {
		...
		unsafe {
			...
			// light properties
			lighting_shader.set_vec3(CStr::from_bytes_with_nul(b"light.ambient\0").unwrap(), 0.2, 0.2, 0.2);
			lighting_shader.set_vec3(CStr::from_bytes_with_nul(b"light.diffuse\0").unwrap(), 0.5, 0.5, 0.5);
			...
			lighting_shader.set_vector3(CStr::from_bytes_with_nul(b"light.position\0").unwrap(), &camera.position.to_vec());
			lighting_shader.set_vector3(CStr::from_bytes_with_nul(b"light.direction\0").unwrap(), &camera.front);
			lighting_shader.set_float(CStr::from_bytes_with_nul(b"light.cutOff\0").unwrap(), f32::cos((12.5 as f32).to_radians()));
			lighting_shader.set_float(CStr::from_bytes_with_nul(b"light.outerCutOff\0").unwrap(), f32::cos((17.5 as f32).to_radians()));

			// set light params
			lighting_shader.set_float(CStr::from_bytes_with_nul(b"light.constant\0").unwrap(), 1.0);
			lighting_shader.set_float(CStr::from_bytes_with_nul(b"light.linear\0").unwrap(), 0.09);
			lighting_shader.set_float(CStr::from_bytes_with_nul(b"light.quadratic\0").unwrap(), 0.032);
